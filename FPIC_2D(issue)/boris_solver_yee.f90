...
contains
    ...
    subroutine Push_Particle(pos_in, vel_in, qm, dt, Ex, Ey, Ez, Bx, By, Bz, &
        ...
        real(8) :: logic_pos(3)
        integer :: i_n, k_n            ! 整数/对齐网格索引
        integer :: i_h, k_h            ! 半整数/偏移网格索引
        integer :: i0, k0
        real(8) :: wxn_0, wxn_1, wzn_0, wzn_1 ! 整数网格权重
        real(8) :: wxh_0, wxh_1, wzh_0, wzh_1 ! 半整数网格权重
        real(8) :: shift_pos
        ...
        ! y 已降为单一物理平面；保留 vy，但不再推进 y 坐标。
        logic_pos(2) = 0.0d0
        logic_pos(3) = (pos_in(3) - grid_origin(3)) / grid_steps(3)
        
        ! 找到包围粒子的立方体单元的左下角索引
		i0 = floor(logic_pos(1)) + 1
		k0 = floor(logic_pos(3)) + 1
        ...
        ! 组装场分量
        ! 2D XZ Yee 插值：Ex/Bz 位于 x 半格，Ez/Bx 位于 z 半格，Ey/By 为中心分量。
        E_loc(1) = (Ex(i_h, 1, k_n)*wzn_0 + Ex(i_h, 1, k_n+1)*wzn_1) * wxh_0 + &
                   (Ex(i_h+1, 1, k_n)*wzn_0 + Ex(i_h+1, 1, k_n+1)*wzn_1) * wxh_1

        E_loc(2) = (Ey(i_n, 1, k_n)*wxn_0 + Ey(i_n+1, 1, k_n)*wxn_1) * wzn_0 + &
                   (Ey(i_n, 1, k_n+1)*wxn_0 + Ey(i_n+1, 1, k_n+1)*wxn_1) * wzn_1

        E_loc(3) = (Ez(i_n, 1, k_h)*wxn_0 + Ez(i_n+1, 1, k_h)*wxn_1) * wzh_0 + &
                   (Ez(i_n, 1, k_h+1)*wxn_0 + Ez(i_n+1, 1, k_h+1)*wxn_1) * wzh_1

        B_loc(1) = (Bx(i_n, 1, k_h)*wxn_0 + Bx(i_n+1, 1, k_h)*wxn_1) * wzh_0 + &
                   (Bx(i_n, 1, k_h+1)*wxn_0 + Bx(i_n+1, 1, k_h+1)*wxn_1) * wzh_1

        B_loc(2) = (By(i_h, 1, k_h)*wzh_0 + By(i_h, 1, k_h+1)*wzh_1) * wxh_0 + &
                   (By(i_h+1, 1, k_h)*wzh_0 + By(i_h+1, 1, k_h+1)*wzh_1) * wxh_1

        B_loc(3) = (Bz(i_h, 1, k_n)*wzn_0 + Bz(i_h, 1, k_n+1)*wzn_1) * wxh_0 + &
                   (Bz(i_h+1, 1, k_n)*wzn_0 + Bz(i_h+1, 1, k_n+1)*wzn_1) * wxh_1
